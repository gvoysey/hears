...
class LogGammachirp(LinearFilterbank):
    ...
    def __init__(self, source, f, b=1.019, c=1, ncascades=4):
        ...
        self.asymmetric_filt_b, self.asymmetric_filt_a = asymmetric_compensation_coeffs(self.samplerate, f,
                                                                                        self.asymmetric_filt_b,
                                                                                        self.asymmetric_filt_a, b, c,
                                                                                        p0, p1, p2, p3, p4)
...
class LinearGaborchirp(FIRFilterbank):
    ...
    def __init__(self, source, f, time_constant, c, phase=0):
        ...
        for ich in xrange(len(f)):
            ...
            self.impulse_response[ich, :] = env * cos(2 * pi * (f[ich] * t + c[ich] / 2 * t ** 2) + phase[ich])
            self.impulse_response[ich, :] = self.impulse_response[ich, :] / sqrt(
                    sum(self.impulse_response[ich, :] ** 2))
    ...
    def __init__(self, source, nchannels, passband, stopband, gpass, gstop, btype, ftype):
        ...
        if btype == 'low' or btype == 'high':
            if len(Wpassband) == 1:  # if there is only one Wn value for all channel just repeat it
                ...
                if len(
                        gstop) != nchannels:  # if the ripple parameters are scalar make them as long as the number of channels
                ...
                if len(filt_b[
                           i]) > order:  # take the highst order of them to be the size of the filter coefficient matrix
                    order = len(filt_b[i])
            ...
            if Wpassband.ndim == 1:  # if there is only one Wn pair of values for all channel just repeat it
                ...
                filt_b, filt_a = [1] * nchannels, [1] * nchannels
                for i in xrange(
                        (nchannels)):  # take the highst order of them to be the size of the filter coefficient matrix
                    filt_b[i], filt_a[i] = signal.iirdesign(Wpassband[:, i], Wstopband[:, i], gpass[i], gstop[i],
                                                            ftype=ftype)
...
class AsymmetricCompensation(LinearFilterbank):
    ...
    def __init__(self, source, f, b=1.019, c=1, ncascades=4):
        ...
        for k in arange(ncascades):
            ...
            r = exp(
                    -p1 * (p0 / p4) ** (
                        k) * 2 * pi * b * ERBw / self.samplerate)  # k instead of k-1 because range 0 N-1
            Df = (p0 * p4) ** (k) * p2 * c * b * ERBw

            phi = 2 * pi * np.maximum((f + Df), 0) / self.samplerate
            psy = 2 * pi * np.maximum((f - Df), 0) / self.samplerate
...
def asymmetric_compensation_coeffs(samplerate, fr, filt_b, filt_a, b, c, p0, p1, p2, p3, p4):
    ...
    for k in arange(nbr_cascade):
        ...
        phi = 2 * pi * np.maximum((fr + Dfr), 0) / samplerate
        psy = 2 * pi * np.maximum((fr - Dfr), 0) / samplerate
