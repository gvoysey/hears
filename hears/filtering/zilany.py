...
class Chirp_Coefficients:
    def __init__(self, cf, taumax, samplerate, rsigma, fcohc):
        ...
        self.Cinitphase = sum(arctan(self.CFmat / (-self.rzeromat)) \
                              ...
                              - arctan(
            (self.CFmat + self.pimg[:, [0, 2, 1, 0, 1]]) / (-self.preal[:, [0, 2, 1, 0, 1]])), axis=1)
        self.CFmat10 = tile(self.CF.reshape(self.nch, -1), 10)
        self.Cgain_norm = prod((self.CFmat10 - self.pimg[:, [0, 1, 2, 3, 4, 5, 0, 3, 1, 5]]) ** 2 + self.preal[:,
                                                                                                    [0, 1, 2, 0, 2, 1,
                                                                                                     0, 0, 1, 1]] ** 2,
                               axis=1)
...
class ZILANY(CombinedFilterbank):
    ...
    def __init__(self, source, cf, update_interval, param={}):
        ...
        updater = Filter_Update([C1_filter], c1_coefficients, samplerate, cf, bmTaumax, bmTaumin, cohc, TauWBMax,
                                TauWBMin)  # instantiation of the updater for the control path
        output = ControlFilterbank(C1_filter, NL2_control, [C1_filter], updater,
                                   update_interval)  # controler for the band pass filter of the control path
